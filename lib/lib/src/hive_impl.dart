...
import 'package:hiveviewerproject/lib/src/backend/storage_backend_memory.dart';
import 'package:hiveviewerproject/lib/src/backend/vm/storage_backend_vm.dart';
...
class HiveImpls extends TypeRegistryImpl implements HiveInterface {
  ...
  /// Not part of public API
  HiveImpls() : _manager = BackendManager() {
    _registerDefaultAdapters();
  }

  /// Not part of public API
  @visibleForTesting
  HiveImpls.debug(this._manager) {
    _registerDefaultAdapters();
  }

  /// Not part of public API
  @visibleForTesting
  HiveImpls.test() : _manager = BackendManager() {
  ...
  Future<BoxBase<E>> _openBox<E>(
    ...
    if (isBoxOpen(name)) {
      ...
      try {
        StorageBackendVm backend;
        if (bytes != null) {
          backend = StorageBackendMemory(bytes, cipher) as StorageBackendVm;
        } else {
          backend = (await _manager.open(
              name, path ?? homePath, recovery, cipher)) as StorageBackendVm;
        }

        BoxBaseImpl<E> newBox;
        if (lazy) {
          newBox = LazyBoxImpl<E>(HiveImpl(), name, comparator, compaction,
              backend as StorageBackend);
        } else {
          newBox = BoxImpl<E>(HiveImpl(), name, comparator, compaction,
              backend as StorageBackend);
        }
